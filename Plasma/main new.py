...
class U_login():
    def __init__(self,root):
        ...
        lbl6 = Label(F2, text="Developed by Sanjeevani",fg="#4863A0", bg="#3b444b", font=("times new roman", 17)).place(x=20, y=420)
...
class User_Page(Tk):
    def __init__(self,*arg):       # Constructors
        ...
        text7.insert(INSERT,("                       Developed By\n\nSanjeevani Sharma\nEmail Id:sanjeevanisaani@gmail.com\n"))
        ...
        if str(self.one_)==str(self.OTP_Forget):
            if len(str(self.passw))>=8:
                if self.passw == self.passconw:
                    ...
                    y= \
                        """UPDATE user SET Password =\"""" + str(self.passw)+\
                        """\""""
                    y=y+" WHERE Phone_No= '"+str(self.phone_2) + "'"
...
class U_Manage(Tk):
    def __init__(self,*arg):       # Constructors
        ...
        self.FM1.place(x=430,y=200,width=700,height=450 )
        ...
        FM12.place(x=0,y=330,relwidth=1,height=80 )

        
        
        btn_update2 = Button(FM12,relief=GROOVE,width=8, font=("times new roman",18,"bold"),bd=6,text="Add",command=self.add).grid(row=0,column=1,pady=6,padx=50,sticky="nesw")        
        ...
        blood_grp.place(x=0,y=190, anchor="w")
        ...
        combo_Donor.place(x=200, y=245, anchor="w")
    ...
    def add(self):
        ...
        if self.Name.get()=="" and self.Email.get()=="" and self.BloodID.get()=="" and self.branchID.get()=="" :
        ...
        if "@" not in self.Email.get():
            ...
            try:    
                ...
                self.conn=sqlite3.connect("plasma.db")
                self.c=self.conn.cursor()
                self.cmd1 = "SELECT W_D from user_details WHERE UID = '" +str(self.UID.get()) + "'"

                self.c.execute(self.cmd1)
                ...
                if self.data:
                    for i in self.data:
                        ...
                        if a=="Yes":
                            ...
                            self.conn=sqlite3.connect("plasma.db")
                            self.c=self.conn.cursor()
                            self.c.execute("SELECT Phone_No FROM user WHERE UID = '" +str(self.UID.get()) + "'")
                            self.data=self.c.fetchall()
                            if self.data:
                                for i in self.data:
                                    self.a=i[0]
                            self.c.execute("INSERT INTO Blood_Donor_Approved (UID, B_Group, Phone_no ) VALUES (?,?,?)",(self.UID.get(),self.BloodID.get(),str(self.a)))
                            self.conn.commit()
                            messagebox.showinfo("Info","Data Added and You are donar  ")
                            self.clear()
                        else:
                            messagebox.showerror("Info","Data Added But Not a Donar")
                        
            except Exception:
                return messagebox.showerror("Error!!","Data already exist ")
    ...
    def reset1(self):
        ...
        if str(self.one_)==str(self.OTP_Forget):
            if len(str(self.passw))>=8:
                if self.passw == self.passconw:
                    y= \
                        """UPDATE hos SET Password =\"""" + str(self.passw)+\
                        """\""""
                    y=y+" WHERE Phone_No= '"+str(self.phone_2) + "'"
                    self.c.execute(str(y))
                    self.conn.commit()
